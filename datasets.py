...
def build_openml_training_specs(
    ...
    if len(specs) < target_n:
        ...
        # Pega o dataset com mais instâncias por nome (dedup por nome base)
        pool = dfs[
            (dfs["NumberOfClasses"].fillna(0) >= 2)
            & (dfs["NumberOfClasses"].fillna(0) <= 50)
            & (dfs["NumberOfInstances"].fillna(0) >= 150)
            & (dfs["NumberOfInstances"].fillna(0) <= 200_000)
            & (dfs["NumberOfFeatures"].fillna(0) >= 2)
            & (dfs["NumberOfFeatures"].fillna(0) <= 5_000)
            & (~dfs["did"].isin(seen_ids))
        ].copy()

        # Desduplicar: para cada nome base, manter apenas o did com mais instâncias
        pool["_name_lower"] = pool["name"].str.lower().str.strip()
        pool = (
            pool.sort_values("NumberOfInstances", ascending=False)
            .drop_duplicates(subset="_name_lower", keep="first")
            .sort_values("NumberOfInstances")  # mais leves primeiro
        )

        skip_substr = (
            "fri_c", "arcene_seed", "analcatdata", "dataset_analcat",
            "test_", "_test", "generated", "synthetic",
        )
